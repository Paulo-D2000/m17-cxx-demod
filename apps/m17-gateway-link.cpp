...
/*
    Main GatewayLink Class
*/
class M17GatewayLink
{
public:
    // Default Address 127.0.0.1 Port 12711
    M17GatewayLink(std::string LocalAddress, unsigned short LocalPort);
    void run();
    int start();
...
// setup
M17GatewayLink::M17GatewayLink(std::string LocalAddress, unsigned short LocalPort){
    addr = LocalAddress;
    port = LocalPort;
}
...
int main(int argc, char* argv[])
{
  ...
  result = _setmode( _fileno( stdout ), _O_BINARY );
   if( result == -1 )
    ...
    int res;
    M17GatewayLink glink(std::string("127.0.0.1"),(unsigned short)17011);
    res = glink.start();
    glink.run();
    return res;
}
